...
/* tableView:cellForRowAtIndexPath: does the following:
 ...
 (3) If the reference to the friend object is non-nil, fills the cell's
     imageView with a call to AFNetworking's extension setImageWithURLRequest:
     with the picture URL and fills the cell's textLabel with the friend object's
     name.
 (4) The image loading success block sets the image in the cell's imageView and
     requests a layout update.  The failure block displays an alert if one
     is not already visible.  User may accept the missing data or attempt to
     refresh the data.
 */
